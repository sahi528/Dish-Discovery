...
Class Actions extends DBConnection{
    ...
    function save_recipe(){
        ...
        // Initialize user_id if session is set
        $_POST['user_id'] = isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 1;
    
        // Escape and prepare data for database insertion
        foreach($_POST as $k => $v){
            if(in_array($k, array('description', 'ingredients', 'step', 'other_info'))) {
                $v = htmlentities($v);
            }
            if(!in_array($k, array('id'))) {
                $v = trim($v);
        ...
        // Construct SQL query based on whether it's an insert or update
        if(isset($cols) && isset($vals)){
            $cols_join = implode(",", $cols);
            $vals_join = implode(",", $vals);
        }
        ...
        if($save){
            ...
            if(empty($id)) {
                ...
                // Retrieve last inserted recipe_id
                $recipe_id = $this->lastInsertRowID();
            } else {
            ...
            if(isset($_FILES['image']) && $_FILES['image']['error'] == UPLOAD_ERR_OK) {
                ...
                if(in_array($type, array('image/png', 'image/jpeg'))) {
                    ...
                    if(move_uploaded_file($upload, $filePath)) {
                        ...
                        $resp['msg'] = "Recipe successfully saved but image upload failed.";
                    }
                } else {
            ...
            // Error in saving/updating recipe
            $resp['status'] = "failed";
            if(empty($id)) {
